...
class GenziiRepository(
    ...
    val ocrScansFlow: Flow<List<OcrScanRecord>> = ocrScanDao.getAllScans().map { list ->
        list.map { entity ->
            OcrScanRecord(
                id = entity.id,
                title = entity.title,
                ...
                wordCount = entity.wordCount,
                charCount = entity.charCount,
                confidencePct = entity.confidencePct,
                language = entity.language,
                formattedDate = entity.formattedDate,
                timestamp = entity.timestamp,
                isSavedToVault = entity.isSavedToVault,
                vaultFileId = entity.vaultFileId,
                imagePath = entity.imagePath
            )
        }
    }

    suspend fun saveOcrScan(
        title: String,
        ...
        val scanTitle = title.trim().ifBlank {
            val firstLine = trimmed.lines().firstOrNull { it.isNotBlank() }?.take(40)?.trim()
            if (!firstLine.isNullOrBlank()) firstLine else "Scanned Document"
        }
        val entity = OcrScanEntity(
            title = scanTitle,
        ...
        ocrScanDao.insertScan(entity)
        return OcrScanRecord(
            id = entity.id,
            title = entity.title,
            extractedText = entity.extractedText,
            wordCount = entity.wordCount,
    ...
    suspend fun deleteOcrScan(id: String) {
        ocrScanDao.deleteScan(id)
    }

    suspend fun saveOcrScanToVault(scanId: String, title: String, content: String): VaultItem {
        saveNoteToVault(title, content, "Scanned")
        val allFiles = vaultDao.getAllFilesSnapshot()
        val created = allFiles.firstOrNull { it.name.startsWith(title) }
        ocrScanDao.updateVaultStatus(scanId, true, created?.id)
        return created?.toVaultItem() ?: VaultItem(
            id = UUID.randomUUID().toString(),
            name = "$title.note",
            category = FileCategory.NOTE,
            sizeString = "1 KB • NOTE",
            content = content
        )
    }
    ...
    suspend fun moveFolder(folderId: String, newParentFolderId: String?, newPath: String) {
        val email = appPreferences.userEmailFlow.first()
        if (firebaseVaultService != null) {
            firebaseVaultService.moveFolder(email, folderId, newParentFolderId, newPath)
        } else {
            vaultFolderDao.moveFolder(folderId, newParentFolderId, newPath)
        }
    }
    ...
    suspend fun deleteOriginalityCheck(id: String) {
        originalityEngine.deleteCheck(id)
    }

    suspend fun clearAllOriginalityChecks() {
        originalityDao.clearAllChecks()
    }
    ...
    suspend fun syncDeviceStorageFiles(): Int {
        ...
        val deviceFiles = StorageFileService.scanAllDeviceStorage(ctx)
        if (deviceFiles.isNotEmpty()) {
            val existingFolders = vaultFolderDao.getAllFolders().first()
            val existingFolderNames = existingFolders.map { it.name.lowercase() }.toSet()
            val newFolders = mutableListOf<VaultFolderEntity>()
            deviceFiles.forEach { file ->
                val folderName = file.folder.trim()
                if (folderName.isNotBlank() &&
                    !existingFolderNames.contains(folderName.lowercase()) &&
                    newFolders.none { it.name.equals(folderName, ignoreCase = true) }
                ) {
                    val colorHex = when (folderName.lowercase()) {
                        "videos", "movies" -> 0xFFEC4899
                        "audio", "music", "podcasts" -> 0xFFA855F7
                        "pictures", "dcim", "camera", "screenshots" -> 0xFF06B6D4
                        "downloads", "download" -> 0xFF10B981
                        "documents", "docs" -> 0xFFF59E0B
                        else -> 0xFF3B82F6
                    }
                    val bgHex = when (colorHex) {
                        0xFFEC4899 -> 0xFFFDF2F8
                        0xFFA855F7 -> 0xFFFAF5FF
                        0xFF06B6D4 -> 0xFFECFEFF
                        0xFF10B981 -> 0xFFECFDF5
                        0xFFF59E0B -> 0xFFFFFBEB
                        else -> 0xFFEFF6FF
                    }
                    newFolders.add(
                        VaultFolderEntity(
                            id = "folder_${folderName.lowercase().replace(" ", "_").replace("/", "_")}",
                            name = folderName,
                            path = "/$folderName",
                            iconColorHex = colorHex,
                            bgColorHex = bgHex,
            ...
            if (newFolders.isNotEmpty()) {
                vaultFolderDao.insertFolders(newFolders)
            }
    ...
    suspend fun getDeviceStorageFiles(): List<VaultItem> {
        val ctx = context ?: return emptyList()
        val deviceFiles = StorageFileService.scanAllDeviceStorage(ctx)
        return deviceFiles.map { it.toVaultItem() }
    }

    suspend fun importDeviceFileToVault(item: VaultItem, targetFolder: String = "Assignments"): VaultItem {
        val entity = VaultFileEntity(
            id = java.util.UUID.randomUUID().toString(),
            name = item.name,
            extension = item.category.extension,
            sizeString = item.sizeString,
            sizeBytes = item.sizeBytes,
            timeAgo = "Imported from storage",
            folder = targetFolder,
    ...
    suspend fun preseedInitialDataIfEmpty() {
        ...
        if (existingFolders.isEmpty()) {
            val defaultFolders = listOf(
                ...
                VaultFolderEntity(id = "folder_documents", name = "Documents", path = "/Documents", iconColorHex = 0xFFF59E0B, bgColorHex = 0xFFFFFBEB, isSecured = false),
                VaultFolderEntity(id = "folder_downloads", name = "Downloads", path = "/Downloads", iconColorHex = 0xFF10B981, bgColorHex = 0xFFECFDF5, isSecured = false),
