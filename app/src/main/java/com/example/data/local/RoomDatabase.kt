...
interface VaultFolderDao {
    ...
    @Query("UPDATE vault_folders SET parentFolderId = :newParentId, path = :newPath WHERE id = :id")
    suspend fun moveFolder(id: String, newParentId: String?, newPath: String)
...
interface OcrScanDao {
    ...
    @Query("SELECT * FROM ocr_scans WHERE id = :id LIMIT 1")
    suspend fun getScanById(id: String): OcrScanEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertScan(scan: OcrScanEntity)

    @Query("UPDATE ocr_scans SET isSavedToVault = :saved, vaultFileId = :vaultId WHERE id = :id")
    suspend fun updateVaultStatus(id: String, saved: Boolean, vaultId: String?)

    @Query("DELETE FROM ocr_scans WHERE id = :id")
    suspend fun deleteScan(id: String)

    @Query("DELETE FROM ocr_scans")
    suspend fun clearAllScans()
}

@Database(
    entities = [
        OriginalityEntity::class,
